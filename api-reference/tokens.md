...
## GET /api/tokens/address/:address/user-balance

Get a user's ETH and token balance for a specific token (cached from indexer).
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `userAddress` | string | User wallet address (0x + 40 hex) |
| `chainId` | integer (min 1) | Chain ID |

**Response:** `ethBalance`, `ethBalanceFormatted`, `tokenBalance`, `tokenBalanceFormatted`, `cached`, optional `needsSync`.
